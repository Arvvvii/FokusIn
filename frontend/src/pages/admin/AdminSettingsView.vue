  ...
  <div class="p-6 md:p-8 xl:p-10 max-w-[1600px] mx-auto animate-in fade-in duration-500">
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-3xl p-7 md:p-8 shadow-[0_10px_40px_rgba(15,23,42,0.06)] border border-white/40 relative overflow-hidden mb-8">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
        <div>
          <h1 class="text-[28px] font-bold text-slate-900 tracking-tight mb-1">System Settings</h1>
          <p class="text-base text-slate-500 font-medium">Konfigurasi keamanan, integrasi API, preferensi sistem, dan parameter platform.</p>
        </div>
        <div class="flex items-center gap-4">
          <button @click="saveChanges" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm shadow-sm hover:bg-indigo-700 transition-all">
            Save Changes
          </button>
        </div>
      </div>
      ...
      <div class="lg:col-span-1">
        <div class="flex flex-col gap-2">
          <button 
            ...
            :class="activeTab === 'security' ? 'bg-white shadow-sm border border-slate-200 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 border border-transparent'"
          ...
          <button 
            @click="activeTab = 'api'"
            :class="activeTab === 'api' ? 'bg-white shadow-sm border border-slate-200 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 border border-transparent'"
            class="px-4 py-3 text-left font-bold text-sm rounded-xl transition-all"
          >
            API & Domain
          </button>
          <button 
            @click="activeTab = 'ai'"
            :class="activeTab === 'ai' ? 'bg-white shadow-sm border border-slate-200 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 border border-transparent'"
            class="px-4 py-3 text-left font-bold text-sm rounded-xl transition-all"
          >
          ...
          <button 
            ...
            :class="activeTab === 'global' ? 'bg-white shadow-sm border border-slate-200 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 border border-transparent'"
            class="px-4 py-3 text-left font-bold text-sm rounded-xl transition-all"
          >
            Global Preferences
          </button>
        ...
        <div v-if="activeTab === 'security'" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div class="space-y-6 max-w-2xl">
            <div>
              ...
              <input type="number" value="120" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 font-medium text-slate-900" />
            </div>

            <div>
              <label class="block text-sm font-bold text-slate-700 mb-2">Maximum Login Attempts</label>
              <input type="number" value="5" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 font-medium text-slate-900" />
            </div>
        ...
        <div v-else-if="activeTab === 'api'" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div class="space-y-6 max-w-2xl">
            <div>
              ...
              <input type="text" value="https://fokusin.edu" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 font-medium text-slate-900" />
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'global'" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div class="space-y-6 max-w-2xl">
            <div>
              ...
              <select class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 font-medium text-slate-900 appearance-none bg-white">
